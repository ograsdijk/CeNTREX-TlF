...
#### Fixed-Basis Compact-Interpolated Lindblad Path

The following chart matches the current
`prepare_lindblad_safe_compact_interpolated_model(...)` implementation. This is
the fixed-basis, Lindblad-safe compact path. The simpler
`prepare_interpolated_effective_model(...)` path remains separate and still
interpolates the dissipator superoperator directly.

```mermaid
flowchart TB
  subgraph Offline["Offline Preprocessing"]
    E["Electric-field patch points<br/>E0, E1, E2, ..."]
    Build["For each patch Ek:<br/>Build exact compact OBE model at field Ek"]
    Local["Local dressed-basis patch objects:<br/>H_internal(Ek)<br/>H_opt(Ek)<br/>H_det(Ek)<br/>full jump/collapse data<br/>Each patch starts in its own local dressed basis"]
    Master["Choose one master/reference patch<br/>and one fixed union layout"]
    Align["Embed each patch into the union state layout<br/>Align/rotate each patch into the same fixed reference basis<br/>Alignment is propagated outward from the master patch"]
    Fixed["Store fixed-basis patch data:<br/>H_internal_fixed(Ek)<br/>H_opt_fixed(Ek)<br/>H_det_fixed(Ek)<br/>K_target_i_fixed(Ek) for each lower target state i<br/>K_recycle_fixed(Ek) for the full recycling channel"]
    Groups["Lower-target bookkeeping in the fixed basis:<br/>active return targets<br/>union of sink target states across all fields<br/>If a target state is absent at one patch, its embedded contribution is zero"]
    Tables["Build/store interpolation tables from fixed-basis patch data"]
    HNote["Hamiltonian interpolation note:<br/>Hermiticity is restored by symmetrization<br/>coherent block structure is preserved<br/>optical coupling is restricted to ground-excited blocks"]
    DNote["Decay interpolation note:<br/>Use convex linear interpolation of PSD fixed-basis kernels<br/>Apply PSD projection after interpolation<br/>Do not interpolate the raw dissipator or full Liouvillian in this Lindblad-safe path"]
    E --> Build --> Local --> Master --> Align --> Fixed --> Tables
    Fixed --> Groups
    Tables --> HNote
    Tables --> DNote
  end

  subgraph Runtime["Runtime Propagation"]
    In["Input:<br/>time-dependent electric field E(t)"]
    Eval["Evaluate prebuilt interpolators at E(t)"]
    HPath["Hamiltonian path:<br/>H_internal(E)<br/>H_opt(E)<br/>H_det(E)"]
    HAnn["Hamiltonian annotations:<br/>Hermitian<br/>block-structure preserving<br/>optical coupling only in ground-excited blocks<br/>detuning follows compact-model block pattern"]
    KPath["Decay path:<br/>K_target_i(E) = convex interpolation of neighboring fixed-basis target kernels<br/>K_recycle(E) = convex interpolation of neighboring fixed-basis full recycling kernels"]
    KAnn["Decay annotations:<br/>Interpolated kernels are PSD-projected<br/>per-target kernels are used for rate bookkeeping and diagnostics<br/>the full recycling kernel is the object used to rebuild Lindblad jumps"]
    Rebuild["Reconstruct c_array(E) from K_recycle(E)<br/>Then build D(E) acting on rho from those jump operators"]
    Combine["d rho/dt = -i[H(E), rho] + D(E) acting on rho"]
    Basis["rho(t) is propagated in one fixed reference basis throughout runtime"]
    Gauge["No gauge-Hamiltonian or basis-motion term:<br/>the propagated basis is fixed in time"]
    Limit["Limitation:<br/>Field-induced mixing among included states appears through H(E), H_opt(E), and D(E)<br/>States excluded from the reduced basis enter only indirectly through patch-derived effective operators<br/>Their populations are not tracked explicitly"]
    In --> Eval
    Eval --> HPath --> HAnn --> Combine
    Eval --> KPath --> KAnn --> Rebuild --> Combine
    Combine --> Basis --> Gauge
    Combine --> Limit
  end
```

What changed relative to the earlier interpretation:

- The Lindblad-safe offline decay storage is not one coarse kernel per decay
  sector. It stores one fixed-basis PSD kernel per lower target state plus one
  fixed-basis full recycling kernel.
- The optically active return manifold is recovered by summing the relevant
  ground-target kernels. Sink targets live in a fixed union layout across all
  fields, and absent targets contribute zeros at that patch.
- The runtime dissipator is rebuilt from the interpolated full recycling kernel.
  The per-target kernels are retained mainly for fixed-basis rate bookkeeping
  and diagnostics.
- The common fixed basis is reached by union-layout embedding plus block
  alignment propagated outward from the master patch. Conceptually this is still
  one fixed reference basis, but it is not implemented purely as a one-shot
  direct rotation of every patch against the master patch.
- The parts that remain correct are: one fixed propagated basis, no
  gauge-Hamiltonian or basis-motion term, coherent mixing of included states
  enters through `H_internal`, `H_opt`, and `H_det`, and excluded states are
  represented only indirectly through the patch-derived effective operators.
